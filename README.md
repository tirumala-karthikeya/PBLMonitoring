...
**Verified against the source data**: for July 2025, School AAKD has enrollment 94 / attendance 133, and the CSV's own derived rate is 0.7074 ("Behind"). `computeAttendanceAggregate()` on that school's `ClassSubjectMetric` rows independently reproduces 70.74% and classifies it "Behind," confirming the engine's formula matches the source data's own methodology without having been told what it was.
...
- **Tier 2**: Monthly Review Summary, covering achievements, MoM changes, priority geographies, and discussion points, generated from the same deterministic engine, with an optional AI narrative layered on top.
...
- **Known build warning**: `next build` logs an Edge Runtime compatibility warning from a transitive dependency of `next-auth` (`jose`'s `CompressionStream` usage). It does not affect functionality, since the JWE code path it warns about isn't used by this app's plain signed-JWT session strategy, but it's worth knowing about if `next-auth`/`jose` are upgraded.
...
- Role-based authorization. There is currently a single account type with full access to every screen; a real deployment would likely add roles (e.g., scoping a donor-facing account to `/reports` only).
